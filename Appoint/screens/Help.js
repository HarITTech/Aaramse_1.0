    ...
    return (
      <Animatable.View animation="fadeInUp" delay={delay} className="mb-4">
        <TouchableOpacity 
          ...
          className={`p-5 rounded-3xl border shadow-sm flex-row items-center justify-between ${
            isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'
          } ${isExpanded ? (isDark ? 'border-blue-900/40' : 'border-blue-100') : ''}`}
        >
          <Text className={`font-black text-base flex-1 pr-4 ${isExpanded ? 'text-blue-500' : (isDark ? 'text-slate-100' : 'text-slate-800')}`}>{question}</Text>
          <MaterialCommunityIcons 
            name={isExpanded ? "minus-circle-outline" : "plus-circle-outline"} 
            size={24} 
            color={isExpanded ? "#3b82f6" : (isDark ? "#475569" : "#94a3b8")} 
          />
        </TouchableOpacity>
        {isExpanded && (
          <Animatable.View animation="fadeInDown" duration={300} className={`p-5 rounded-b-3xl -mt-4 pt-8 ${isDark ? 'bg-slate-950/40' : 'bg-blue-50/30'}`}>
            <Text className={`leading-6 font-medium ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>{answer}</Text>
          </Animatable.View>
        )}
      </Animatable.View>
    );
  ...
  return (
    <View className={`flex-1 ${isDark ? 'bg-[#020617]' : 'bg-slate-50'}`}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 60 }}>
        <LinearGradient
          colors={isDark ? ['#0a0f1d', '#020617'] : ['#fff', '#f1f5f9']}
          className={`px-8 pt-12 pb-12 rounded-b-3xl shadow-sm mb-8 border-b ${isDark ? 'border-slate-800' : 'border-slate-100'}`}
        >
          <TouchableOpacity 
            onPress={() => navigation.goBack()}
            className={`mb-6 w-10 h-10 items-center justify-center rounded-xl shadow-sm border ${
              isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100'
            }`}
          >
          ...
          <Animatable.View animation="fadeIn">
            <Text className={`text-3xl font-black mb-2 ${isDark ? 'text-white' : 'text-slate-900'}`}>Help Center</Text>
            <Text className={`font-medium text-base leading-6 ${isDark ? 'text-slate-400' : 'text-slate-505'}`}>Everything you need to know about AaramSe.</Text>
          </Animatable.View>
        </LinearGradient>

        <View className="px-6">
          <Text className={`text-xs font-bold uppercase tracking-widest mb-6 ml-4 ${isDark ? 'text-slate-500' : 'text-gray-400'}`}>Common Questions</Text>
          ...
          <View className={`mt-8 p-6 rounded-3xl shadow-lg ${
            isDark ? 'bg-slate-900 border border-slate-800' : 'bg-blue-900'
          }`}>
            <Text className="text-white text-xl font-black mb-2">Still Stuck?</Text>
            <Text className={`font-medium mb-8 ${isDark ? 'text-slate-400' : 'text-blue-100/70'}`}>Our human support team is available 24/7 to assist you with any inquiries.</Text>
